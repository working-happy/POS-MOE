...
from my_bert.CNN import CNNnet
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.pos_embeddings = nn.Embedding(8, config.hidden_size)
    ...
    def forward(self,poslabel_ids, input_ids, token_type_ids=None):   # input_ids  [16, 128]
        seq_length = input_ids.size(1)
        position_ids = torch.arange(seq_length, dtype=torch.long, device=input_ids.device)
        position_ids = position_ids.unsqueeze(0).expand_as(input_ids)  # position_ids  [16, 128]  按顺序编号
        ...
        pos_embeddings = self.pos_embeddings(poslabel_ids)

        embeddings = words_embeddings + position_embeddings + token_type_embeddings + pos_embeddings
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self,poslabel_ids, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True):
        ...
        embedding_output = self.embeddings(poslabel_ids, input_ids, token_type_ids)
...
class MTCCMBertForMMTokenClassificationCRF(BertPreTrainedModel):
    ...
    def __init__(self, config, layer_num1=1, layer_num2=1, layer_num3=1,  num_labels=2, auxnum_labels=2):
        ...
        self.self_attention_visual = BertSelfEncoder(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.vismap2text = nn.Linear(2048, config.hidden_size)
        self.vismap2text_visual = nn.Linear(200, config.hidden_size)
        self.vismap2text_v2 = nn.Linear(2048, config.hidden_size)
        self.vismap2text_v2_visual = nn.Linear(200, config.hidden_size)
        ...
        self.entity_experts_layer = nn.ModuleList(
            [nn.Linear(2 * config.hidden_size, num_labels) for _ in range(num_labels)])

        self.entity_experts_layer_aux = nn.ModuleList(
            [nn.Linear(config.hidden_size, auxnum_labels) for _ in range(auxnum_labels)])

        self.moee_linear = nn.Linear(2 * config.hidden_size, num_labels)

        self.moee_linear_aux = nn.Linear(config.hidden_size, auxnum_labels)

    def MoEE(self, hidden_states):      # (b, len, 2h)
        """Mixture of Entity Experts"""
        model_cnn = CNNnet()
        entity_embedding_final = np.load('/home/chenjianying/multimodalNER/visual_embedding/UMT/data/entity_embedding_final_2017.npy',
                                         allow_pickle=True)
        entity_embedding_final = torch.from_numpy(entity_embedding_final)
        x = Variable(entity_embedding_final)

        out = model_cnn(x)     # (13, 1536)

        out = out.transpose(0, 1)   # (1536, 13)

        # print(out.shape)

        m = []
        hidden_states = hidden_states.transpose(0, 1)   # (len, b, 2h)
        for h in hidden_states: #shape of h - (b, 2h)

            expt = [self.dropout(l(h)) for l in self.entity_experts_layer]
            expt = torch.stack(expt).transpose(0, 1)    # （b, ner, ner）

            # alpha = nn.Softmax(self.dropout(self.moee_linear(h)))  # (b, ner, ner)
            alpha = nn.Softmax(torch.add(torch.mul(self.dropout(self.moee_linear(h)), 0.7), torch.mul(h.mm(out.cuda()), 0.3)))    # (b, ner)     torch.mul(out.cuda(), 0.5)

            T = torch.bmm(expt.reshape(expt.shape[0], expt.shape[1], expt.shape[2]),
                          alpha.dim.view(alpha.dim.shape[0], alpha.dim.shape[1], 1))[:, :, -1]

            # alpha = nn.Softmax(dim = 1)  # (b, ner)
            # alpha1 = alpha(self.dropout(self.moee_linear(h)))

            # Old
            # T = torch.bmm(expt.reshape(expt.shape[0], expt.shape[2], expt.shape[1]), alpha.dim.view(alpha.dim.shape[0], alpha.dim.shape[1], 1))[:, :, -1]
            # New
            # T = torch.bmm(expt.reshape(expt.shape[0], expt.shape[1], expt.shape[2]), alpha1.view(alpha1.shape[0], alpha1.shape[1], 1))[:, :, -1]

            m.append(T)     # （len, b, ner）
        m = torch.stack(m)
        m = m.transpose(0, 1)     # (b, len, ner)
        return m

    def MoEE_aux(self, hidden_states):      # (b, len, h)
        m = []
        hidden_states = hidden_states.transpose(0, 1)   # (len, b, h)
        for h in hidden_states: #shape of h - (b, h)

            expt = [self.dropout(l(h)) for l in self.entity_experts_layer_aux]
            expt = torch.stack(expt).transpose(0, 1)    # （b, ner, ner）

            alpha = nn.Softmax(self.dropout(self.moee_linear_aux(h)))  # (b, ner, ner)
            T = torch.bmm(expt.reshape(expt.shape[0], expt.shape[1], expt.shape[2]),
                          alpha.dim.view(alpha.dim.shape[0], alpha.dim.shape[1], 1))[:, :, -1]
    ...
    # visual_embeds_att [8, 2048, 7, 7]   img_emb  (8, 4, 200)
    def forward(self, poslabel_ids, input_ids, segment_ids, input_mask, added_attention_mask, trans_matrix,
                img_emb, added_input_mask_visual, labels=None, auxlabels=None):
        # Get the emission scores from the BiLSTM
        sequence_output, _ = self.bert(poslabel_ids, input_ids, token_type_ids=segment_ids, attention_mask=input_mask,
                                       output_all_encoded_layers=False)
        ...
        aux_addon_sequence_output = aux_addon_sequence_encoder[-1]             # [8, 128, 768]
        aux_bert_feats = self.aux_classifier(aux_addon_sequence_output)
        #######aux_bert_feats = self.aux_classifier(sequence_output)
        # aux_bert_feats = self.MoEE_aux(aux_addon_sequence_output)
        ...
        main_addon_sequence_output = main_addon_sequence_encoder[-1]       #  [8, 128, 768]

        ##############      视觉嵌入     ##########################

        # img_emb (8, 4 ,200)
        converted_vis_embed_map_visual = self.vismap2text_visual(img_emb)  # self.batch_size, 4, hidden_dim

        img_mask_visual = added_input_mask_visual[:,:4]
        extended_img_mask_visual = img_mask_visual.unsqueeze(1).unsqueeze(2)
        extended_img_mask_visual = extended_img_mask_visual.to(dtype=next(self.parameters()).dtype) # fp16 compatibility
        extended_img_mask_visual = (1.0 - extended_img_mask_visual) * -10000.0

        converted_vis_embed_map_visual = self.self_attention_visual(converted_vis_embed_map_visual, extended_img_mask_visual)
        converted_vis_embed_map_visual = converted_vis_embed_map_visual[-1]

        # 文本main_addon_sequence_output [8, 128, 768] 图像converted_vis_embed_map_visual [8, 4, 768] m 图像模态
        # extended_img_mask_visual [8,1,1,4]
        cross_encoder_visual = self.txt2img_attention(main_addon_sequence_output, converted_vis_embed_map_visual, extended_img_mask_visual)
        cross_output_layer_visual = cross_encoder_visual[-1]  # self.batch_size * text_len * hidden_dim

        # apply img2txt attention mechanism to obtain multimodal-based text representations
        converted_vis_embed_map_v2_visual = self.vismap2text_v2_visual(img_emb)  # self.batch_size, 4, hidden_dim
        cross_txt_encoder_visual = self.img2txt_attention(converted_vis_embed_map_v2_visual, main_addon_sequence_output,
                                                   extended_txt_mask)
        # extended_txt_mask [8,1,1,128]       文本中间模态
        cross_txt_output_layer_visual = cross_txt_encoder_visual[-1]  # self.batch_size * 4 * hidden_dim

        cross_final_txt_encoder_visual = self.txt2txt_attention(main_addon_sequence_output, cross_txt_output_layer_visual,
                                                         extended_img_mask_visual)
        cross_final_txt_layer_visual = cross_final_txt_encoder_visual[-1]  # self.batch_size * text_len * hidden_dim  文本模态

        ########################      UMT门控   ##############################
        # merge_representation_visual = torch.cat((cross_final_txt_layer_visual, cross_output_layer_visual), dim=-1)
        # gate_value_visual = torch.sigmoid(self.gate(merge_representation_visual))  # batch_size, text_len, hidden_dim
        # gated_converted_att_vis_embed_visual = torch.mul(gate_value_visual, cross_output_layer_visual)
        #
        # final_output_visual = torch.cat((cross_final_txt_layer_visual, gated_converted_att_vis_embed_visual), dim=-1)
        # # self.batch_size * text_len * 2 * hidden_dim
        ########################      UMT门控   ##############################

        ########################      2018门控   #############################
        multimodal_features = self.gmf(cross_final_txt_layer_visual,  cross_output_layer_visual)       # [batch_size, max_seq_len, hidden_dim]
        final_output_visual = self.filtration_gate(main_addon_sequence_output, multimodal_features)
        ...
        bert_feats = self.classifier(final_output_visual)   # (b, len, ner)

        bert_feats_Moee = self.MoEE(final_output_visual)

        alpha = 0.5
        final_bert_feats = torch.add(torch.mul(bert_feats, alpha),torch.mul(trans_bert_feats, 1-alpha))  # (b, len, ner)
        final_bert_feats_Moee = torch.add(torch.mul(bert_feats_Moee, alpha),torch.mul(trans_bert_feats, 1 - alpha))  # (b, len, ner)

        # final_bert_feats = softmax(final_bert_feats, dim=-1)
        ...
        if labels is not None:
            ...
            Moee_loss = - self.crf(final_bert_feats_Moee, labels, mask=input_mask.byte(), reduction='mean')
            loss = main_loss + beta*aux_loss + Moee_loss
            return loss
        else:
            pred_tags = self.crf.decode(final_bert_feats_Moee, mask=input_mask.byte())
            return pred_tags
